...
import Business.Data.UserDirectory;
import Interface.IApplicationInterface;
import Interface.IUserInteraction;

import MainProject.AppMain;
...
public class Drivers implements IApplicationInterface, IUserInteraction
{
			...
			case '2': //Manage files
				do { // Inner loop
					...
					if (manage == 'a') {//Create a file
						...
						FileHandlers.CreateFile(currentDirectory, createFile);
						continue;
					} 
					else if (manage == 'b') {//Delete a file
						String deleteFile = Drivers.GetUserFileName(in);
						FileHandlers.DeleteFile(currentDirectory, deleteFile);
						continue;
					}
					else if (manage == 'c') //Search for file
					{
						...
						DirectoryManager.GetDirectoryFiles(currentDirectory, fileList);
						FileHandlers.SearchFile(currentDirectory, path, fileList);
						continue;
					}
					else if (manage == 'x') {
	...
	{
		final String APPNAME = "Company Lockers"; //Application name
		final String DEV = "Developed by Pooja N Venkatesh"; //Developer details
